...
def main():
    ...
    os.makedirs(definitions.PREDICTED_MASKS_UNETPLUSPLUS_DIR, exist_ok=True)
    os.makedirs(definitions.PREDICTED_OVERLAYS_UNETPLUSPLUS_DIR, exist_ok=True)
    ...
    print(f"Carregando modelo de: {definitions.PLUSPLUS_MODEL_PATH}")
    ...
    model.load_state_dict(torch.load(definitions.PLUSPLUS_MODEL_PATH, map_location=torch.device(device)))
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for image_name in tqdm(image_files, desc="Processando Imagens (U-Net++)"):
            ...
            output_mask_path = os.path.join(definitions.PREDICTED_MASKS_UNETPLUSPLUS_DIR, image_name)
            output_mask_image.save(output_mask_path)

            # Cria e salva o overlay.
            resized_original_img = np.array(original_img.resize((config.PLUSPLUS_IMAGE_WIDTH, config.PLUSPLUS_IMAGE_HEIGHT)))
            ...
            output_overlay_path = os.path.join(definitions.PREDICTED_OVERLAYS_UNETPLUSPLUS_DIR, overlay_name)
            output_overlay_image.save(output_overlay_path)

    print(f"\nInferência do U-Net++ concluída.")
    print(f"Máscaras salvas em: {definitions.PREDICTED_MASKS_UNETPLUSPLUS_DIR}")
    print(f"Overlays salvos em: {definitions.PREDICTED_OVERLAYS_UNETPLUSPLUS_DIR}")
